...
**Architecture**: `app.py` is a thin orchestrator — just page setup and the
tab layout. Each brand's Streamlit UI lives in its own `<brand>_ui.py` file
(its raw-file loader, category-mapping loader, and `render()` function), and
each brand's actual conversion logic lives in its own `<brand>_mapping.py`.
This means `app.py` stays a fixed ~30 lines no matter how many brands get
added — adding a new brand only means adding a new pair of files plus one
import + one tab line in `app.py`. See "Adding a new brand" at the bottom.

- `app.py` — page config + `st.tabs(...)` layout only.
- `ui_common.py` — shared helpers used by every brand's UI file: `file_bytes`,
  `rows_to_xlsx_bytes`, `PLATFORM_LABELS`, `CATEGORY_SHEET_PLATFORM`.
- `hydro_flask_ui.py` / `herschel_ui.py` / `toms_ui.py` / `image_combiner_ui.py`
  — each tab's Streamlit UI (file uploaders, previews, download buttons) and
  raw/category-mapping-file loaders.
- `mapping.py` — Hydro Flask's conversion logic (raw row → each
  marketplace's columns). Also holds the handful of truly brand-agnostic
  helpers (image joining, HTML conversion, spec parsing) that the other
  two mapping modules import and reuse.
- `herschel_mapping.py` — Herschel's conversion logic — its own
  category-matching rule (exact match on Product Type + Specific Category +
  Gender) and material-extraction logic.
- `toms_mapping.py` — Toms' conversion logic — a THIRD category-matching rule
  (Gender-in-Title AND Words-in-Title as Title keywords, combined with an
  exact match on Specific Category), fixed Material=Rubber specs, and the
  same image-based Zalora ColorFamily classification approach as Herschel's.
...
Each brand is a self-contained pair of files, so adding one never touches
the other brands. Fastest path: copy the brand closest to the new one's
needs as a starting point (Hydro Flask's keyword-in-title matching,
Herschel's exact-tuple matching, or Toms' hybrid title+exact matching), then
adjust for whatever's different.

1. **Copy a `<brand>_mapping.py`** (e.g. `herschel_mapping.py`) to
   `<newbrand>_mapping.py`. Update the category-matching rule if it's
   different, the output headers if the real templates differ, and any
   brand-specific defaults (fixed specs, material handling, etc.). Import
   brand-agnostic helpers from `mapping.py` rather than duplicating them.
2. **Copy a `<brand>_ui.py`** (e.g. `herschel_ui.py`) to `<newbrand>_ui.py`.
   Update the raw-column list/labels, the category-mapping-file loader if
   the format differs, and the `render()` function's text/download filenames.
   Import from `ui_common.py` for the shared bits.
3. **Copy a `build_<brand>_template.py`** to build the new brand's blank
   input templates, and run it once to generate the `.xlsx` files.
4. **In `app.py`**: add `import <newbrand>_ui` and one more
   `with tool_tabN: <newbrand>_ui.render()` line. That's the only change
   to the shared file.

Since every brand so far has turned out to need a genuinely different
category-matching rule and column layout, expect the investigation (getting
the real templates, figuring out the matching rule, testing against real
data) to be most of the work — the file-structure part above is mechanical.
